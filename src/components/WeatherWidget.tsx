...
const WeatherWidget: React.FC<WeatherWidgetProps> = ({ variant = "default" }) => {
    ...
    if (variant === "compact") {
        return (
            <div className="flex items-center gap-5 bg-slate-900/95 backdrop-blur-2xl border-2 border-primary/60 hover:border-primary rounded-full px-6 py-3.5 transition-all duration-300 group cursor-default shadow-[0_12px_48px_rgba(0,0,0,0.6)]">
                <div className="flex items-center gap-4">
                    <div className="group-hover:scale-125 transition-transform duration-500">
                        {getWeatherIcon(weather.weathercode, weather.is_day === 1, 24)}
                    </div>
                    <div className="flex flex-col leading-none">
                        <span className="text-[18px] font-black text-white tracking-tighter drop-shadow-md">
                            {weather.temperature}°C
                        </span>
                    </div>
                </div>
                <div className="w-[3px] h-6 bg-primary/30 rounded-full" />
                <div className="flex flex-col items-start leading-none gap-1">
                    <div className="flex items-center gap-1.5">
                        <MapPin size={10} className="text-[#D4AF37] animate-pulse" />
                        <span className="text-[11px] text-[#D4AF37] font-black uppercase tracking-[0.2em] drop-shadow-md">Namhsan</span>
                    </div>
                    <span className="text-[10px] text-white/90 font-bold uppercase tracking-wider">{getWeatherDesc(weather.weathercode)}</span>
                </div>
            </div>
        );
    }
