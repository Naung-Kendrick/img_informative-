...
export default function HotNewsTicker() {
    ...
    return (
        <div className="w-full bg-white border-b border-slate-200 flex items-center overflow-hidden h-10 sm:h-12 relative z-30 card-shadow">
            ...
            <div className="bg-primary text-white font-bold px-4 sm:px-8 h-full flex items-center justify-center shrink-0 z-10 shadow-[4px_0_15px_rgba(0,0,0,0.1)] gap-1.5 sm:gap-2 uppercase tracking-[0.1em] sm:tracking-[0.15em] text-[11px] sm:text-[12px]">
                    ...
                    <div className="flex items-center h-full shrink-0">
                        {breakingNews.map((news, idx) => (
                            <div key={`news-1-${news._id}-${idx}`} className="flex items-center mx-10">
                                ...
                                <Link to={`/news/${news._id}`} className="text-[13px] sm:text-[15px] font-bold text-slate-700 hover:text-primary transition-colors tracking-tight padauk-bold cursor-pointer">
                                    {news.title}
                                </Link>
                            </div>
                        ))}
